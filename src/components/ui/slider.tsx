    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="border-primary/50 ring-ring/30 block size-5 shrink-0 rounded-full border-2 bg-background shadow-lg transition-all duration-200 hover:scale-110 hover:shadow-xl focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
