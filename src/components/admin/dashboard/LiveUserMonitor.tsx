    ...
    <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
      <CardHeader className="bg-card border-b border-border py-6 px-8 flex flex-row items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <CardTitle className="text-xl font-semibold text-foreground">Live Sessions</CardTitle>
        </div>
      ...
      <CardContent className="p-0">
          ...
          {activeUsers.map((user) => (
            <div key={user.id} className="p-5 flex items-center justify-between hover:bg-muted/30 transition-colors group">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold text-xs border border-primary/20">
                  {user.name.split(" ").map(n => n[0]).join("")}
                </div>
                <div>
                  <p className="text-sm font-bold text-foreground flex items-center gap-2">
                    {user.name}
                    <span className="text-[10px] text-muted-foreground flex items-center gap-1 bg-muted px-1.5 py-0.5 rounded uppercase font-medium">
              ...
              <div className="text-right">
                <p className="text-xs font-semibold text-foreground">{user.duration}</p>
        ...
        <div className="p-4 bg-muted/20 text-center">
          <button className="text-[11px] font-semibold text-primary hover:text-primary/80 uppercase tracking-widest flex items-center justify-center gap-1 w-full transition-all">
