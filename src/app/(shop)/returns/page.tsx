  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <Link href="/profile" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground mb-6"><ArrowLeft className="w-4 h-4" />Back to Profile</Link>

        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-3xl font-bold">Returns & Refunds</h1><p className="text-muted-foreground">Track your return requests</p></div>
        </div>
        ...
        {loading ? (
          ...
          <Card className="text-center py-12">
            <CardContent>
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-muted flex items-center justify-center"><RefreshCw className="w-8 h-8 text-muted-foreground" /></div>
              <h2 className="text-xl font-bold mb-2">No Returns Yet</h2>
              <p className="text-muted-foreground mb-6">When you request a return, it will appear here.</p>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={cn("w-10 h-10 rounded-full flex items-center justify-center", config.color)}><Icon className="w-5 h-5" /></div>
                        <div><p className="font-medium">Order #{ret.orderId?.orderNumber || ret.orderId?._id?.slice(-6)}</p><p className="text-sm text-muted-foreground">Requested {formatDate(ret.createdAt)}</p></div>
                      </div>
                      <Badge variant="outline" className={cn("gap-1", config.color)}>{config.label}</Badge>
                    </div>
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      {ret.items.length > 2 && <p className="text-sm text-muted-foreground">+{ret.items.length - 2} more items</p>}
                    </div>

                    <div className="flex items-center justify-between pt-4 border-t">
                      <div className="text-sm"><span className="text-muted-foreground">Reason: </span><span className="font-medium">{ret.reason}</span></div>
                      <div className="flex items-center gap-4">
                        <span className="font-bold text-green-600">${ret.refundAmount.toFixed(2)}</span>
                        <Button variant="ghost" size="sm" asChild><Link href={`/returns/${ret._id}`}>View Details<ChevronRight className="w-4 h-4 ml-1" /></Link></Button>
                      </div>
                    </div>
                    ...
                    {ret.notes.length > 0 && (
                      <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                        ...
                        <p className="text-sm text-muted-foreground">{ret.notes[ret.notes.length - 1].message}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
