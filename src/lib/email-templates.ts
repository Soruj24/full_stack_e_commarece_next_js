  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
      ...
      <div style="margin: 32px 0; text-align: center;">
        <a href="${verificationLink}" style="background: linear-gradient(to right, #3b82f6, #8b5cf6); color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      ...
      <p style="color: #9ca3af; font-size: 12px; text-align: center;">This link will expire in 24 hours. If you didn't create an account, please ignore this email.</p>
    </div>
  `;
}
...
export function invitationTemplate(inviteLink: string, inviterName: string) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
      ...
      <div style="margin: 32px 0; text-align: center;">
        <a href="${inviteLink}" style="background: linear-gradient(to right, #10b981, #3b82f6); color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">Accept Invitation</a>
      </div>
      <p style="color: #6b7280; font-size: 14px;">Or copy and paste this link in your browser:</p>
      ...
      <hr style="border: 0; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
      <p style="color: #9ca3af; font-size: 12px; text-align: center;">This invitation link will expire in 7 days.</p>
    </div>
  `;
}

export function orderStatusTemplate(orderNumber: string, status: string, trackingLink?: string) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
      ...
      ${trackingLink ? `<div style="margin: 32px 0; text-align: center;"><a href="${trackingLink}" style="background: #3b82f6; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">Track Your Order</a></div>` : ""}
      <p style="color: #6b7280; font-size: 14px; margin-top: 24px;">Thank you for shopping with us!</p>
    </div>
  `;
}

export function promotionTemplate(title: string, message: string, link: string) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
      <h2 style="color: #1f2937; margin-bottom: 16px;">${title}</h2>
      <p style="color: #4b5563; line-height: 1.5;">${message}</p>
      <div style="margin: 32px 0; text-align: center;">
        <a href="${link}" style="background: linear-gradient(to right, #f59e0b, #ef4444); color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">Check It Out</a>
      </div>
    </div>
  `;
}

export function abandonedCartTemplate(name: string, items: { name: string; quantity: number; price: number }[]) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px; border: 1px solid #e5e7eb; border-radius: 24px; background-color: #ffffff;">
      ...
      <div style="text-align: center;">
        <a href="${process.env.NEXT_PUBLIC_APP_URL}/cart" style="background: #3b82f6; color: white; padding: 16px 32px; text-decoration: none; border-radius: 12px; font-weight: bold; display: inline-block;">Complete Your Purchase</a>
      </div>
      <p style="color: #9ca3af; font-size: 12px; text-align: center; margin-top: 40px;">If you didn't mean to leave these items, you can ignore this email.</p>
    </div>
  `;
}
